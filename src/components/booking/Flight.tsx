...
export default function Flight() {
    ...
    async function handleSubmit(e: FormEvent){
       ...
       const res = await fetch("/api/flight",{
        method: 'POST',
        body: JSON.stringify({destination,origin,startDate,endDate})
       })
    ...
    <>
       <h3 className="text-2xl font-semibold text-blue-700 mb-4">Flights</h3>
         <form onSubmit={handleSubmit} method='POST'>
              ...
              <ul className="bg-white border border-gray-300 rounded-lg shadow-lg mt-1 w-full max-h-40 overflow-y-auto">
              ...
              <ul className="bg-white border border-gray-300 rounded-lg shadow-lg mt-1 w-full max-h-40 overflow-y-auto">
                {destinationSuggestions.map((airport, index) => (
                  ...
                  >
                    ...
                    <div>
                        <label htmlFor="flight-return" className="block text-gray-700 font-medium mb-2">Return Date</label>
                        <input type="date" id="flight-return" name='endDate' className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
                <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg shadow-md transform hover:scale-105 transition-all duration-300 ease-in-out">
                    Search Flights
                </button>
         </form>
